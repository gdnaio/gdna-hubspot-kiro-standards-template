...
{"productId":"$PRODUCT_ID","version":"$VERSION","projectType":"$PROJECT_TYPE","slug":"$SLUG","domain":"$DOMAIN","artifact":"$ZIP","publishedAt":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","publishedBy":"manual","repository":"$(git remote get-url origin 2>/dev/null || echo local)","commitSha":"$(git rev-parse HEAD 2>/dev/null || echo none)"}
EOF
...
curl -sf -X POST "$API/ops/products/$PRODUCT_ID/versions" -H 'Content-Type: application/json' -d "{\"version\":\"$VERSION\",\"projectType\":\"$PROJECT_TYPE\",\"slug\":\"$SLUG\"}" --no-progress-meter || true
if [ "$PROJECT_TYPE" = "landing-page" ]; then
  curl -sf -X POST "$API/ops/landing-pages/$SLUG/deploy" -H 'Content-Type: application/json' -d "{\"source\":\"products/$PRODUCT_ID/$VERSION/$ZIP\",\"version\":\"$VERSION\",\"deployedBy\":\"manual\"}" --no-progress-meter || echo "Deploy failed"
fi
echo "Done: $S3/$ZIP"
